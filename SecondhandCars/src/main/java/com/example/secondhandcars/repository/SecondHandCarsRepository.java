...
public interface SecondHandCarsRepository extends JpaRepository<SecondHandCars,Integer>{
		...
		//DML Delete Query
			@Modifying
			@Query("delete from SecondHandCars s where s.model=?1")
			public int deleteCarByModel(String model);
			
		//update query
			@Modifying
			@Query("update  SecondHandCars s set s.colour=?1 where s.model=?2")
			public int updateCarByModel(String colour,String model);
